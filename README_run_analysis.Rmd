...
<b>Activities measured shown in video: https://www.youtube.com/watch?v=XOEN9W05_4A 

<p>(i) The code should work if you have run_analysis.R and the unzipped "UCI HAR Dataset" data folder present in the working directory of R/Rstudio. Otherwise, paths need to be set, for example (as commented code):
...
1.2 *It was discovered that the descriptions of activity features are actually not in a right format for use as variable 'names' so the script resolves this by "making names" out of the vector-column of character strings from activityF. <br>This will enable one to apply the select() function subsequently. This also allows the measurement data to eb appropriately labelled with descriptive variable names. 
