...
The code should work if you have run_analysis.R and the unzipped "UCI HAR Dataset" data folder present in the working directory of R/Rstudio. Otherwise, paths need to be set, e.g.:
...
Load the required dplr package.
## --- LIBRARIES --------------------------------------------------------------
```{r}
library(dplyr)
```
